...
st.info(""" ***Завдання.*** Провести перевірочний розрахунок системи аварійного зливу ЛЗР самопливом з вертикального циліндричного резервуара, що наведена на рисунку 4.1. 
За необхідності запропонувати обгрунтовані розрахунком заходи, що забезпечують умови виконання аварійного зливу. 
Вид рідини; робоча температура, $t_р$, [$^oС$]; густина $/rho$, [$кг/м^3$]; об’єм рідини в резервуарі $V$, [$м^3$]; робочий тиск, [$kPa$]; 
площа поперечного перерізу резервуара $F$, [$м^2$]; діаметр аварійного трубопроводу, $d_вн$, [$мм$]; матеріал аварійного трубопроводу; 
тип пуску системи пуск системи обирають за варіантом. """)
...
st.write("Програма для визначення параметрів трубопроводу та часу зливу рідини.")

# Секція 1: Геометрія та об'єм
with st.expander("Параметри резервуара та Гідравліка", expanded=True):
    Vp = st.number_input("Робочий об'єм рідини Vp, м³", value=15.0)
    F_res = st.number_input("Площа перерізу резервуара F, м²", value=5.0)
    H1 = st.number_input("Висота H1 (початок зливу), м", value=4.0)
    H2 = st.number_input("Висота H2 (кінець зливу), м", value=0.5)
