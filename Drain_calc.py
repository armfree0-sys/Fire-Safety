...
col_d1d2_table, col_d1d2_image = st.columns([1, 1])
...
with col_d1d2_table:
    ...
    st.markdown("**Таблиця 5.3** — Коефіцієнти звуження")
    data = {
...
with col_d1d2_image:
    try:
        # Додаємо порожній простір зверху (опціонально), якщо хочете відцентрувати відносно таблиці
        st.write("##") # Це додає невеликий відступ зверху
...
with col_text1:
    st.markdown("""
    ### Значення місцевих опорів для коліна:
    * **1** — Апарат, що спорожнюється (резервуар)
    * **4** — Аварійний трубопровід
    * **5** — Гідрозатвор (враховано як місцевий опір $\\xi_г = 3$)
    * **6** — Аварійна ємність (підземна)
