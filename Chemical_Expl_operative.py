...
def calculate_time_to_target(distance_km, v_wind):
    ...
    return (distance_km * 1000) / (v_wind * 60)
...
if app_mode == "ℹ️ Головна":
    ...
    3. **📍 ГІС-модуль:** Вибір координат кліком по карті Google.
    4. **🕒 Таймінг:** Розрахунок часу до підходу хмари.
    """)
...
# --- РЕЖИМ 2: ПРОГНОЗУВАННЯ (РУЧНИЙ) ---
    ...
    if target_dist <= g_final:
        ...
        col3.success("Об'єкт поза зоною ураження")
